#!/usr/bin/env python3
"""
BLEU Score Calculator: QWEN RAG Concise vs OpenAI Baseline
...
def calculate_sentence_bleu(candidate: str, reference: str) -> Dict[str, float]:
    ...
    try:
        ...
        bleu_4 = sentence_bleu([reference_tokens], candidate_tokens, smoothing_function=smoothing)
...
def save_results(results: List[Dict], stats: Dict):
    ...
    if pd is not None:
        ...
        csv_path = RESULTS_DIR / "per_question_bleu_concise.csv"
        df.to_csv(csv_path, index=False)
        print(f"  ✅ Saved detailed results to: {csv_path}")
    else:
        json_path = RESULTS_DIR / "per_question_bleu_concise.json"
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f"  ✅ Saved detailed results to: {json_path}")
    
    summary_path = RESULTS_DIR / "summary_metrics_concise.json"
...
def print_summary(stats: Dict):
    ...
    print("\n" + "="*70)
    print("BLEU Score Summary: QWEN RAG Concise vs OpenAI Baseline")
    print("="*70)
...
def main():
    ...
    print("="*70)
    print("BLEU Score Calculator: QWEN RAG Concise vs OpenAI Baseline")
    print("="*70)
