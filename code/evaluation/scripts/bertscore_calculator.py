#!/usr/bin/env python3
"""
BERTScore Calculator: QWEN vs OpenAI Answers
Compares QWEN-generated answers (candidate) against OpenAI-generated answers (reference)
...
# Directories
QWEN_DIR = Path("/home/himanshu/dev/output/qwen")
OPENAI_DIR = Path("/home/himanshu/dev/test/data/processed/qa_pairs_individual_components")
...
def load_bleu_results_for_comparison() -> Optional[Dict]:
    ...
    """
    bleu_csv = RESULTS_DIR / "per_question_bleu.csv"
...
def save_results(results: List[Dict], stats: Dict):
    ...
    if pd is not None:
        ...
        csv_path = RESULTS_DIR / "per_question_bertscore.csv"
        df.to_csv(csv_path, index=False)
        print(f"  ✅ Saved detailed results to: {csv_path}")
    else:
        # Fallback: save as JSON
        json_path = RESULTS_DIR / "per_question_bertscore.json"
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f"  ✅ Saved detailed results to: {json_path}")
    
    # Save summary statistics
    summary_path = RESULTS_DIR / "summary_bertscore.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Calculate BERTScore for QWEN vs OpenAI answers")
    ...
    parser.add_argument("--qwendir", type=Path, default=QWEN_DIR,
                       help="Directory containing QWEN answer files")
    parser.add_argument("--openaidir", type=Path, default=OPENAI_DIR,
                       help="Directory containing OpenAI answer files")
    ...
    print("="*70)
    print("BERTScore Calculator: QWEN vs OpenAI Answers")
    print("="*70)
    ...
    # Load data
    qwen_data = load_qwen_answers(args.qwendir)
    openai_data = load_openai_answers(args.openaidir)
