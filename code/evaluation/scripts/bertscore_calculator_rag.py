#!/usr/bin/env python3
"""
BERTScore Calculator: QWEN RAG vs OpenAI Baseline
...
def calculate_bertscore_scores(
    ...
    candidates = [pair['qwen_rag_answer'] for pair in matched_pairs]
...
def save_results(results: List[Dict], stats: Dict):
    ...
    if pd is not None:
        ...
        csv_path = RESULTS_DIR / "per_question_bertscore_rag.csv"
        df.to_csv(csv_path, index=False)
        print(f"  ✅ Saved detailed results to: {csv_path}")
    else:
        # Fallback: save as JSON
        json_path = RESULTS_DIR / "per_question_bertscore_rag.json"
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        print(f"  ✅ Saved detailed results to: {json_path}")
    
    # Save summary statistics
    summary_path = RESULTS_DIR / "summary_bertscore_rag.json"
...
def print_summary(stats: Dict):
    ...
    """
    print("\n" + "="*70)
    print("BERTScore Summary: QWEN RAG vs OpenAI Baseline")
    print("="*70)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Calculate BERTScore for QWEN RAG vs OpenAI baseline")
    ...
    parser.add_argument("--qwendir", type=Path, default=QWEN_RAG_DIR,
                       help="Directory containing QWEN RAG answer files")
    parser.add_argument("--openaidir", type=Path, default=OPENAI_BASELINE_DIR,
                       help="Directory containing OpenAI baseline answer files")
    ...
    print("="*70)
    print("BERTScore Calculator: QWEN RAG vs OpenAI Baseline")
    print("="*70)
    ...
    # Load data
    qwen_data = load_qwen_rag_answers(args.qwendir)
    openai_data = load_openai_baseline_answers(args.openaidir)
